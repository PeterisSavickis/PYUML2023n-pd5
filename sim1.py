...
class Bear:
    ...
    def tryToBreed(self):
        ...
        self.__energy -= 10  # Energy cost for breeding
        if self.__energy <= 0:
            self.__world.delThing(self)
            return
...
def mainSimulation():
    ...
    for i in range(worldLifeTime):
        myWorld.liveALittle()
        fish_count, bear_count = myWorld.count_creatures()
        fishCountList.append(fish_count)
        bearCountList.append(bear_count)

    with open('simulation_data.txt', 'w') as file:
        file.write("Time, Fish, Bears\n")
        for i in range(worldLifeTime):
